...
\begin{align*}
t_i &= \lceil t_i^* \rceil \\
t^*_i &\sim \mathrm{Weibull}\left(\lambda_i, k\right) \\
...
```{r}
sim_data |>
	dplyr::mutate(x_jitter = x + rnorm(nrow(sim_data), 0, 0.1)) |>
	ggplot2::ggplot() +
	aes(x = x_jitter, y = t, group = (x)) +
	geom_point(position = ggbeeswarm::position_beeswarm())
#ggbeeswarm::geom_beeswarm(aes(y = t_star), color = "gray")
	geom_segment(
		aes(x = x, xend = x, y = t, yend = t_star)
	)
