...
dealing with our censored $y$ values.

```{r}
mod_2b <- cmdstanr::cmdstan_model(here::here("Ex2b.stan"), compile = FALSE)
mod_2b$compile(pedantic = TRUE, force_recompile = TRUE)
```

```{r}
writeLines(readLines(here::here("Ex2b.stan")))
```

We need to get the data into the correct form again to pass into the Stan
program. The form we need this time is thankfully much closer to the
typical data frame format in R.
